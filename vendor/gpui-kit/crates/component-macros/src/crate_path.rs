...
/// dependency required by kit consumers. The `gpui-box` package fallback
...
pub(crate) fn gpui() -> syn::Result<TokenStream> {
    match crate_name("gpui-kit") {
        ...
        Err(kit_error) => crate_name("gpui-box")
            .map(found_crate_path)
            .map_err(|gpui_error| {
                syn::Error::new(
                    Span::call_site(),
                    format!(
                        "IntoPlot requires a direct dependency on `gpui-kit` or `gpui-box`: \
                         gpui-kit lookup failed: {kit_error}; gpui-box lookup failed: {gpui_error}"
                    ),
                )
            }),
    }
