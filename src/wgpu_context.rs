...
impl WgpuContext {
    ...
    /// Creates a GPUI graphics context around resources owned by the host.
    ///
    /// The supplied device and queue are retained verbatim. This allows an
    /// embedding application to submit GPUI work and its own rendering work
    /// to the same wgpu device without staging pixels through CPU memory.
    #[cfg(not(target_family = "wasm"))]
    pub fn from_external(
        ...
        log::info!(
            "Using host GPU adapter: name={:?}, backend={:?}, device_type={:?}, vendor={:#06x}, \
             device={:#06x}",
            adapter_info.name,
        ...
        Ok(Self {
            ...
            device,
            queue,
            backend: WgpuBackend::Native(adapter_info.backend),
            dual_source_blending,
            color_texture_format,
            device_lost,
            last_error,
