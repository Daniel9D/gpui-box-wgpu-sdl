...
impl WgpuRenderer {
    ...
    fn new_headless_with_format(
        ...
        if !format_features.allowed_usages.contains(required_usages) {
            anyhow::bail!(
                "Adapter {:?} cannot use target format {:?} with required usages {:?}",
                context.adapter.get_info().name,
                surface_format,
                required_usages,
            );
...
impl WgpuHeadlessRenderer {
    ...
    /// Creates a headless GPUI renderer on a device and queue owned by the host.
    ...
    /// Renders directly into a texture view allocated by the host application.
